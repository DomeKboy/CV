...
namespace ChessEngine
{
    ...
    {
        public Bishop(Color color, int curRow, int curCol, bool actpeaceHasAlreadyMoved) : base(color, curRow, curCol, actpeaceHasAlreadyMoved)
        {
        }
        ...
        public override bool IsMoveLegal(int targetRow, int targetCol, Board actboard)
        {
            ...
            //ha nem jó a logika(átlón kívüli mezők)
            else
            {
                return false;
            }
        ...
        public override List<int[,]> PotencialMoves(Board actBoard, Color actColor)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    ...
                    {
                        //megnézzük h nem-e saját bábunk áll ott // A SZÍNT MÉG LEHET VÁLTOZTATNI KELL
